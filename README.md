...
## Описание
CardReaderApp — это десктопное приложение с графическим интерфейсом на Kotlin (Jetpack Compose Desktop) и бизнес-логикой на Java для взаимодействия с ридерами смарт-карт (SIM/USIM) через PC/SC-интерфейс. Приложение позволяет искать подключённые ридеры, подключаться к ним, отправлять APDU-команды и читать данные с SIM-карт.
...
│   │   │       └── Main.kt         # Главный UI приложения (Kotlin, Compose Desktop)
│   │   ├── java/
│   │   │   └── com/sexysalve/cardreaderapp/
│   │   │       └── CardReaderBackend.java # Вся логика работы с ридером, APDU и картой (Java)
...
- **Kotlin**: только для UI (Jetpack Compose Desktop)
- **Java**: основной язык для backend/логики работы с ридером, APDU и картой (CardReaderBackend.java)
...
## Основные ��лассы и функции backend (Java)
